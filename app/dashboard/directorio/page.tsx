  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-6 py-4 flex justify-between items-center">
        ...
        <button onClick={() => router.push("/dashboard")} className="text-sm px-4 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg">Volver al Dashboard</button>
      </header>

      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex justify-between items-end mb-6">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Directorio Comercial</h1>
            <p className="text-slate-600 dark:text-slate-400">Acceso restringido: Gestión de contactos externos.</p>
          </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
              {contactos.map(c => (
                <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/20">
                  <td className="p-4 font-semibold text-slate-900 dark:text-white">{c.nombre_completo}</td>
                  <td className="p-4"><p className="text-slate-900 dark:text-white">{c.empresa}</p><p className="text-xs text-slate-500">{c.cargo}</p></td>
                  <td className="p-4 text-sm text-slate-600 dark:text-slate-300"><p>{c.email}</p><p>{c.telefono}</p></td>
        ...
        {showContactoModal && (
          <div className="fixed inset-0 flex items-center justify-center p-4 bg-slate-900/60 z-50">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-md">
              ...
              <form onSubmit={handleSubmitContacto} className="space-y-4 text-black dark:text-white">
                <input type="text" placeholder="Nombre Completo" required value={formDataContacto.nombre_completo} onChange={e => setFormDataContacto({...formDataContacto, nombre_completo: e.target.value})} className="w-full p-2 border rounded dark:bg-slate-700" />
                <input type="text" placeholder="Empresa" value={formDataContacto.empresa} onChange={e => setFormDataContacto({...formDataContacto, empresa: e.target.value})} className="w-full p-2 border rounded dark:bg-slate-700" />
                <input type="text" placeholder="Cargo" value={formDataContacto.cargo} onChange={e => setFormDataContacto({...formDataContacto, cargo: e.target.value})} className="w-full p-2 border rounded dark:bg-slate-700" />
                <input type="email" placeholder="Email" value={formDataContacto.email} onChange={e => setFormDataContacto({...formDataContacto, email: e.target.value})} className="w-full p-2 border rounded dark:bg-slate-700" />
                <input type="text" placeholder="Teléfono" value={formDataContacto.telefono} onChange={e => setFormDataContacto({...formDataContacto, telefono: e.target.value})} className="w-full p-2 border rounded dark:bg-slate-700" />
